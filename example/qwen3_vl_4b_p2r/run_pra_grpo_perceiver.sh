...
model_path="$MNT_PATH/models/Qwen/Qwen3-VL-4B-Instruct"
project_name="qwen3_vl_4b_p2r"
exp_name="run_pra_grpo_perceiver"
...
# ─── Frozen reasoner service (for perceiver reward) ────────────────────────────
REASONER_HOST="${REASONER_HOST:-YOUR_REASONER_HOST}"
REASONER_PORT=${REASONER_PORT:-8000}
...
python3 -m verl.trainer.main_ppo \
    ...
    +reward.custom_reward_function.reward_kwargs.role=perceiver \
    +reward.custom_reward_function.reward_kwargs.reasoner_host="$REASONER_HOST" \
    +reward.custom_reward_function.reward_kwargs.reasoner_port=$REASONER_PORT \
    +reward.custom_reward_function.reward_kwargs.reasoner_model="$REASONER_MODEL" \
    ...
    +reward.custom_reward_function.reward_kwargs.max_pixels=$MAX_PIXELS \
    +reward.custom_reward_function.reward_kwargs.image_prefix="$MNT_PATH/datasets" \
