...
class RayPPOTrainer:
    ...
    def _dump_generations(self, inputs, outputs, gts, scores, reward_extra_infos_dict, dump_path):
        ...
        for i in range(n):
            ...
            lines.append(json.dumps(entry, ensure_ascii=False, default=_default))
    ...
    def _validate(self, merged: bool = False):
        ...
        for test_data in self.val_dataloader:
            ...
            # Free heavy intermediate objects to avoid memory accumulation across val batches
            for obj in (test_batch, test_gen_batch, test_gen_batch_padded, test_output_gen_batch_padded, test_output_gen_batch):
                if hasattr(obj, "non_tensor_batch"):
                    if "multi_modal_inputs" in obj.non_tensor_batch:
                        del obj.non_tensor_batch["multi_modal_inputs"]
                    for key in list(obj.non_tensor_batch.keys()):
                        if "image" in key.lower() or "pixel" in key.lower():
                            del obj.non_tensor_batch[key]
            if "multi_modal_inputs" in test_data:
                del test_data["multi_modal_inputs"]
            ...
            del reward_tensor, reward_extra_info, test_data
        ...
        # Free validation accumulation lists and force memory return to OS
        del data_source_lst, sample_inputs, sample_outputs, sample_gts, sample_scores, sample_uids, sample_turns
        del reward_extra_infos_dict, data_sources
        import gc
        gc.collect()
        try:
            import ctypes
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # gen_batch is never used again after union — free its image data now
                    if "multi_modal_inputs" in gen_batch.non_tensor_batch:
                        del gen_batch.non_tensor_batch["multi_modal_inputs"]
                    for key in list(gen_batch.non_tensor_batch.keys()):
                        if "image" in key.lower() or "pixel" in key.lower():
                            del gen_batch.non_tensor_batch[key]
                    # Also free the original batch_dict images (shared refs with gen_batch)
                    if "multi_modal_inputs" in batch_dict:
                        del batch_dict["multi_modal_inputs"]
                    ...
                    if self.config.trainer.critic_warmup > self.global_steps:
                        ...
                        # Free heavy multi-modal pixel data after training forward passes are done
                        for obj in (batch, gen_batch_output):
                            if "multi_modal_inputs" in obj.non_tensor_batch:
                                del obj.non_tensor_batch["multi_modal_inputs"]
                            for key in list(obj.non_tensor_batch.keys()):
                                if "image" in key.lower() or "pixel" in key.lower():
                        ...
                        if self.config.trainer.save_freq > 0 and (
                            ...
                            # Checkpoint gathering creates large temporary CPU allocations
                            # (full state_dict + optimizer state). Force return to OS.
                            gc.collect()
                            try:
                                import ctypes
                                ctypes.CDLL("libc.so.6").malloc_trim(0)
                ...
                # Explicitly free large objects from this step to prevent cross-step accumulation
                del batch, gen_batch, gen_batch_output, reward_tensor, reward_extra_infos_dict, batch_dict
                try:
                    del old_log_prob, ref_log_prob, actor_output
                except NameError:
                    pass
                try:
                    del critic_output, values
                except NameError:
                    pass
                gc.collect()
                try:
