...
@dataclass
class HRBenchSample(BaseDatasetSample):
    data_dir: str = ""

    def __post_init__(self):
        self.image = self.row["image"]
        options = [
            f"{letter}. {self.row[letter]}"
            for letter in ["A", "B", "C", "D"]
            if self.row.get(letter) is not None and str(self.row[letter]).strip()
        ]
        if options:
            self.row["question"] += "\nOptions:\n" + "\n".join(options)
    ...
    def build_prompt(self, pre_key=None):
        ...
        prompt_text = self.prompt_template[pre_key].format(question=self.row["question"])
        prompt_text += self.prompt_template["mca_post_prompt"]
        return prompt_text

    def build_perceive_prompt(self):
        prompt = self.prompt_template["perceive_pre_prompt"].format(question=self.row["question"])
...
class HRBenchEvaluator(BaseDatasetEvaluator):
    sample_cls = HRBenchSample

    def get_desc(self):
        return "Evaluating HR-Bench"
    ...
    def aggregate_results(self, results):
        ...
        for bench, type_df in results_df.groupby("benchmark"):
            group = {"overall": {"accuracy": float(type_df["accuracy"].mean()) * 100}}
            if has_bbox_count:
                group["overall"]["bbox_count"] = float(type_df["bbox_count"].mean())
            for q_type, cat_df in type_df.groupby("question_type"):
                group[q_type] = {"accuracy": float(cat_df["accuracy"].mean()) * 100}
                if has_bbox_count:
                    group[q_type]["bbox_count"] = float(cat_df["bbox_count"].mean())
            output[bench] = group
